#!/usr/bin/env python3
"""
ClaudeStrike with MCP Integration for Kali Linux (COST OPTIMIZED)
Usage: python claude_chat_cost.py [--mcp] [--mcp-server URL]
By: Christopher M. Burkett DBA: CyberAndFires
GitHub: https://github.com/ChrisBurkett/claudestrike

Cost Optimizations:
- Prompt caching for repeated context
- Smart output trimming (only essential info)
...
class ClaudeStrike:
    """Main CLI application with cost optimization"""
    ...
    def __init__(self, use_mcp: bool = False, mcp_server: str = "http://localhost:5000"):
        ...
        # Cached context for cost savings
        self.cached_context = """You are ClaudeStrike, an AI pentesting assistant.
You help with Kali Linux security testing, analyzing tool outputs, and providing guidance.
Be concise and focus on actionable information."""
    ...
    def trim_output(self, output: str, max_lines: int = 100) -> str:
        """Trim command output to reduce token usage"""
        lines = output.split('\n')
        if len(lines) <= max_lines:
            return output
        
        # Keep first 50 and last 50 lines
        trimmed = '\n'.join(lines[:50]) + f"\n\n... [trimmed {len(lines) - 100} lines] ...\n\n" + '\n'.join(lines[-50:])
        print_cost_info(f"Output trimmed from {len(lines)} to 100 lines to save costs")
        return trimmed
    ...
    def run_command(self, command: str) -> str:
        ...
        combined = ""
        if output:
            # Trim output for cost savings
            combined += self.trim_output(output)
        if errors:
            combined += f"\n[stderr]: {errors[:500]}"  # Limit stderr too
    ...
    def summarize_conversation(self):
        ...
        print_cost_info("Summarizing conversation history to reduce costs...")
        
        try:
            # Create a summary of older messages
            messages_to_summarize = self.conversation[:-4]  # Keep last 4 messages
            
            summary_request = [{
                "role": "user",
                "content": f"Summarize this conversation in 2-3 sentences: {messages_to_summarize}"
            }]
    ...
    def chat(self, user_message: str) -> str:
        ...
        # Summarize every 10 messages to save costs
    ...
    def run(self):
        ...
        print(f"{Colors.BOLD}{Colors.GREEN}║{Colors.RESET}  {Colors.BOLD}{Colors.YELLOW}💰 COST OPTIMIZED MODE{Colors.RESET}                             {Colors.BOLD}{Colors.GREEN}║{Colors.RESET}")
        ...
        print(f"\n{Colors.BOLD}{Colors.YELLOW}  💸 Cost Savings Active:{Colors.RESET}")
        print(f"{Colors.YELLOW}  ├─{Colors.RESET} Prompt caching (90% off repeated context)")
        print(f"{Colors.YELLOW}  ├─{Colors.RESET} Output trimming (long results shortened)")
        print(f"{Colors.YELLOW}  └─{Colors.RESET} Auto-summarization (every 10 messages)")
...
def main():
    parser = argparse.ArgumentParser(
        description="ClaudeStrike - AI pentesting assistant (Cost Optimized)"
    )
    parser.add_argument(
        "--mcp",
        action="store_true",
    ...
    app = ClaudeStrike(use_mcp=args.mcp, mcp_server=args.mcp_server)
    app.run()
